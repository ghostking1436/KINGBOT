...
def configure_ngrok():
    ngrok_token = "2pS3n2zmIgHVxdkzbWG5zzrajvG_4sLAbtG3X76ve9SyAbVSJ"
    try:
...
def update_soul_txt(public_url):
    with open("lucifermbk1.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in lucifermbk1.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("lucifermbk1.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated lucifermbk1.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update lucifermbk1.txt on VPS: {str(e)}")
